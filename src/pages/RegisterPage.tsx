...
import { Mail, Lock, Eye, EyeOff, Loader2, CheckCircle, AlertCircle, ArrowRight, User, Phone } from 'lucide-react';
...
export const RegisterPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        setStatus({ type: 'success', message: data.message || 'Registration successful! Redirecting to login...' });
      ...
      >
        ...
        <div className="bg-white/80 backdrop-blur-xl border border-slate-200/60 rounded-3xl shadow-xl p-8 sm:p-10">
          ...
          <div className="text-center mb-8">
            ...
            <h1 className="text-2xl font-black text-slate-800 mt-4 tracking-tight">Create Account</h1>
            <p className="text-sm text-slate-500 font-medium mt-1">Sign up for free and start exploring</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full bg-white border border-slate-200 rounded-xl pl-11 pr-4 py-3 text-sm font-semibold outline-none focus:border-cyan-500 focus:ring-2 focus:ring-cyan-500/20 text-slate-800 shadow-sm transition-all"
                />
              </div>
            </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full bg-white border border-slate-200 rounded-xl pl-11 pr-12 py-3 text-sm font-semibold outline-none focus:border-cyan-500 focus:ring-2 focus:ring-cyan-500/20 text-slate-800 shadow-sm transition-all"
                />
                <button
                  type="button"
            ...
            {/* Confirm Password */}
            <div>
              <label className="text-xs font-extrabold text-slate-700 block mb-1.5">Confirm Password</label>
              <div className="relative">
                <Lock className="absolute left-4 top-1/2 -translate-y-1/2 w-4.5 h-4.5 text-slate-400" />
                <input
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full inline-flex items-center justify-center gap-2 bg-gradient-to-r from-cyan-500 to-indigo-500 hover:from-cyan-600 hover:to-indigo-600 text-white font-extrabold text-sm py-3.5 rounded-xl shadow-lg shadow-cyan-500/25 transition-all active:scale-[0.98] disabled:opacity-60 disabled:cursor-not-allowed cursor-pointer mt-2"
            >
              {loading ? (
                <>
